...
export default function EditJob() {
      ...
      try {
        ...
        if (jobData.is_recurring) {
          ...
          setRecurringOptions({
            ...
            days: jobData.recurring_days || [],
            endDate: jobData.recurring_end_date || "",
          })
        }
        ...
        toast({
          title: "Error",
          description: language === "en" ? "Failed to load job details" : "Не вдалося завантажити деталі роботи",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  const handleRecurringOptionChange = (name: string, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.service_type || !formData.description || !formData.location) {
      toast({
        title: "Error",
        description:
          language === "en" ? "Please fill in all required fields" : "Будь ласка, заповніть всі обов'язкові поля",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: language === "en" ? "Job updated successfully" : "Роботу успішно оновлено",
      })
      ...
      toast({
        ...
        description:
          language === "en"
            ...
            : "Не вдалося оновити роботу. Спробуйте ще раз.",
        variant: "destructive",
      })
    } finally {
  ...
  // Translations
  const pageTitle = language === "en" ? "Edit Job" : "Редагувати завдання"
  ...
  const updateJobLabel = language === "en" ? "Update Job" : "Оновити завдання"
  const updatingLabel = language === "en" ? "Updating..." : "Оновлення..."
  const loadingLabel = language === "en" ? "Loading..." : "Завантаження..."
    ...
    return (
      <div className="app-container">
        <header className="p-4 flex items-center bg-card shadow-medium z-10">
          <Link href="/my-jobs/hirer" className="p-2 rounded-full hover:bg-muted transition-colors duration-200">
        ...
        <div className="flex items-center justify-center h-[80vh]">
          <div className="flex flex-col items-center">
            <div className="h-8 w-8 border-4 border-t-transparent border-primary rounded-full animate-spin mb-4"></div>
            <p>{loadingLabel}</p>
          </div>
        </div>
        <BottomNavigation />
      ...
      <div className="app-container">
        ...
        <div className="flex items-center justify-center h-[80vh]">
          <div className="text-center">
            ...
            <Button onClick={() => router.push("/my-jobs/hirer")}>
              {language === "en" ? "Back to My Jobs" : "Назад до моїх завдань"}
            </Button>
          </div>
        </div>
        <BottomNavigation />
